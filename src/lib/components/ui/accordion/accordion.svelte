	...
	{#each items as item, i}
		<Accordion.Item value="${i}" class="group bg-[#151516] mb-4 rounded-[8px] p-0">
			<Accordion.Header class="p-0">
				<Accordion.Trigger
					class="flex w-full flex-1 items-center justify-between py-5 text-xl font-medium transition-all [&[data-state=open]>span>svg]:rotate-45 [&[data-state=closed]]:text-neutral-60 pt-4 pb-6 px-6"
				>
					<p class="text-xl font-powerGroteskBold font-bold">{item.title}</p>
					...
					>
						...
						<Plus class="transition-all duration-200" />
					</span>
				</Accordion.Trigger>
			</Accordion.Header>
			...
			<Accordion.Content
				...
				class="pb-6 text-base tracking-[-0.01em] px-6"
			>
