...
<ul class="hidden lg:block pl-12 w-1/5 sticky top-24 space-y-4 text-sm max-h-[calc(100vh-6rem)] overflow-y-auto font-hubot">
	...
	>
		...
		{#if !isOpen}
			<div class="p-3 bg-secondary w-full">
				<div class="w-11/12 overflow-hidden whitespace-nowrap text-ellipsis">
					{tableOfContents.find((item) => item.id === currentHash)?.title ||
						tableOfContents[selectedItemIndex].children.find((child) => child.id === currentHash)?.title}
				</div>
			</div>
		{/if}
